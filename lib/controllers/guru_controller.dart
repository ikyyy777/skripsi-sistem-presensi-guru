...
class GuruController extends GetxController {
  ...
  void startBackgroundTask() {
    backgroundTask = Timer.periodic(const Duration(seconds: 1), (Timer timer) async {
      await fetchDataGuru();
      presensiModel.value = await fetchDataPresensi();
    });
  }
  ...
  Future<void> kirimPresensi() async {
    try {
      ...
      if (doc.exists) {
        Map<String, dynamic> presensiData = doc.data() as Map<String, dynamic>;
        ...
        if (presensiData.containsKey(yearNow)) {
          ...
          if (monthData != null) {
            ...
            if (alreadyPresensi) {
              ...
              return; // Keluar dari fungsi jika sudah presensi
            }
